"""Aviator M6 Streamlit Cloud frontend; model inference stays on the VM."""
...
try:
    BACKEND_API_URL = st.secrets.get("BACKEND_API_URL", "")
except Exception:
    BACKEND_API_URL = ""
BACKEND_API_URL = (os.getenv("BACKEND_API_URL") or BACKEND_API_URL or "https://aviator-m6.duckdns.org").rstrip("/")
...
.hero h1 { margin:.35rem 0; font-size:2.2rem; }
...
def call_prediction(rounds: list[float]) -> dict[str, Any] | None:
    try:
        ...
        if response.status_code != 200:
            st.error(f"Backend returned HTTP {response.status_code}: {response.text[:300]}")
            return None
        return response.json()
    except requests.RequestException as exc:
...
st.markdown(f"<section class='hero'><div class='kicker'>AVIATOR M6 / REMOTE INFERENCE</div><h1>Calibrated multiplier analytics</h1><p>Streamlit Cloud is the presentation layer. The private M6 model and Groq integration remain on the VM.</p></section>", unsafe_allow_html=True)
...
with left:
    ...
    default = "1.12, 1.25" if mode == "Two rounds" else "1.25"
    text = st.text_input("Chronological multipliers", value=default, help="Use one or two values, for example 1.12, 1.25")
    analyze = st.button("Analyze next round", type="primary", use_container_width=True)
    if analyze:
        try:
            rounds = [float(part.strip()) for part in text.split(",") if part.strip()]
            if len(rounds) not in (1, 2):
                raise ValueError("Enter one or two multiplier values.")
...
with right:
    ...
    if result:
        ...
        m1, m2 = st.columns(2)
        m1.metric("Model", prediction["model_type"].replace("_", " ").title())
        m2.metric("Confidence", prediction["confidence_level"])
        st.markdown(f"**Estimated range:** `{prediction['estimated_range']}`")
        st.bar_chart({key:value for key,value in result["probabilities"].items()}, height=230)
